...
namespace PlotPCA {
	public partial class Form1 : Form {
		...
		private double[,] excludePoints(double[,] initialPointList, double[,] toRemovePoints)
		{
			...
			for (int i = 0; i < initialPointList.Rows(); i++) {
				...
				for (int j = 0; j < toRemovePoints.Rows(); j++) {
					if (initialPointList[i, 0] == toRemovePoints[j, 0] && initialPointList[i, 1] == toRemovePoints[j, 1]) {
						exists = true;
						break;
					}
				}
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			int cloud2points = 200;
			int cloud3points = 300;
			...
			Console.Clear();
			var a = getRandomPoints(allData, 10);
			allData = excludePoints(allData, a);

			int[] indexesInitial = new int[10];
			for (int i = 0; i < 10; i++) {
